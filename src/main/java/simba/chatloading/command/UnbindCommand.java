...
public class UnbindCommand {
    public static int UnbindExecute(CommandContext<CommandSourceStack> context) {
        if (BindInstance.Unbind(context.getArgument(BIND_KEY, String.class))) {
            context.getSource().sendSuccess(() -> Component.translatable("chat.chatae2.unbind.success"), false);
            return 1;
        } else {
            context.getSource().sendSuccess(() -> Component.translatable( "chat.chatae2.unbind.failed"), false);
            return 0;
        }
    }
}
